...
from app.models import db, UserModel, MenuModel, PermissionModel, RoleModel, \
    PermissionMenuModel, PermissionRoleModel, RoleUserModel, RoleRoleModel
...
def generate_user():
    for i in range(1, 11):
        user = UserModel(id=i, name=f'ying{i}', password='123', remark=f'ying{i}')
        db.session.add(user)

        # 给用户都赋予一个普通用户角色
        role_user = RoleUserModel(role_id=1, user_id=i)
...
def generate_menu():
    fake_menus = [
        ('首页', '/', 0, 1),
        ('新增项目', '/add/project', 1, 0),
        ('项目详情', '/project/detail', 2, 1),
        ('修改项目', '/project/update', 3, 0),
        ('删除项目', '/project/delete', 4, 0),
        ('修改用户', '/user/update', 5, 0),
        ('删除用户', '/user/delete', 6, 0)
    ]

    for fake_menu in fake_menus:
        menu = MenuModel(name=fake_menu[0], url=fake_menu[1], order=fake_menu[2], common=fake_menu[3])
        db.session.add(menu)
    db.session.commit()
